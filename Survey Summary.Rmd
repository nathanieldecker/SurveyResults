...
multi_resp_plot <- function(x, s = NULL, t = NULL, sub_reduce = 0, title_reduce = 0, ...){
  x %>% gather("kind", "val") %>% ggplot(aes(kind, fill = val, alpha = val)) + geom_bar(position = "fill") +
    aes(reorder(stringr::str_wrap(kind, 30),val)) + scale_alpha_manual(values=c(0,1)) +
    scale_fill_manual(values=c('grey92', "grey40")) + 
    theme(axis.title.x = element_blank(), legend.position = "none") + 
    scale_y_continuous(labels = scales::percent) + coord_flip() + labs(subtitle = wrapper(s, subtitle_width - sub_reduce), title = wrapper(t, title_width - title_reduce))
}
srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("RP")), isnt.na) %>% 
  ...
  transmute(`As a residence for self or family member(s)`       = RP_1,
            ...
            Other                                               = RP_10) %>%
  gather("kind", "val") %>% ggplot(aes(kind, fill = val, alpha = val)) + geom_bar(position = "fill") + aes(reorder(stringr::str_wrap(kind, 30),val)) + scale_alpha_manual(values=c(0,1)) + scale_fill_manual(values=c('grey92', "grey50")) +
    theme(axis.title.x = element_blank(), legend.position = "none") + scale_y_continuous(labels = scales::percent) + coord_flip() + 
...
response_by_port <- srpos %>% dplyr::count(`Portfolio Size of Owner`) %>% rename(Number = n)
```

```{r reponse_by_portfolio, results="asis"}
pandoc.table(response_by_port, caption="Most Respondents had more than 2, but less than 50 units",
             justify = "lr", # style = "multiline",
             missing = " ", big.mark = ',', split.table = Inf)
```
...
This summary uses three kinds of charts to present the responses to the survey questions. All charts have **subtitle** that identifies the survey question. Some charts also have a **title** that summarizes a conclusion that I think can be made from the responses to the question. The chart below is a **simple bar chart**. I use this chart to summarize the responses to some multiple choice questions, particularly questions where the owner could check multiple answers. For this reason the sum of the percentages will often add up to more than 100%. In these charts the length of each bar represents the proportion of survey takers who choose each answer. So, for example, about 58% of respondents reported owning a "single family detached house", whereas only about 10% reported owning a "single family attached house, rowhouse or townhouse." In many instances not everyone who took the survey answered the question so the total number of respondants is included at the bottom of the chart. In this case 1,023 respondants answered this question.
...
srpos %>% mutate(TYPE = factor(recode(TYPE, "Co-operativeunit above" = "Co-operative",
  ...
  multi_resp_bar(TYPE, s = "What type of unit is the surveyed property?")
```
...
srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("RP")), isnt.na) %>% 
  ...
  transmute(`As a residence for self or family member(s)`       = RP_1,
            ...
            `As future security for family member(s)`           = RP_9,
            Other                                               = RP_10) %>%
  multi_resp_plot(s = "Why did the owner acquire this property?")
```

<center> 
<span style="color:grey">n = `r srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("RP")), isnt.na) %>% 
  select(starts_with("RP")) %>% filter(RP_1 | RP_2 | RP_3| RP_4| RP_5| RP_6| RP_7| RP_8| RP_9| RP_10) %>% nrow()`</span>
</center>
...
srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("Q130")), isnt.na) %>% 
  select(starts_with("Q130")) %>% filter(Q130_1 | Q130_2 | Q130_3| Q130_4| Q130_5| Q130_6| Q130_7) %>% 
  transmute(`MLS listings`       = Q130_1,
            ...
            Other                 = Q130_7) %>% multi_resp_plot(s= "When selecting this property for purchase what services did the owner use?")
```
<center> 
<span style="color:grey">n = `r srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("Q130")), isnt.na) %>% 
  select(starts_with("Q130")) %>% filter(Q130_1 | Q130_2 | Q130_3| Q130_4| Q130_5| Q130_6| Q130_7) %>% nrow()`</span>
</center>
...
srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("RO")), isnt.na) %>% 
  select(starts_with("RO")) %>% filter(RO_1 | RO_2 | RO_3| RO_4| RO_5| RO_6| RO_7|RO_8|RO_9|RO_10|RO_11|RO_12)  %>% 
  transmute(`As a residence for self or family member(s)`       = RO_1,
            ...
            Other = RO_12) %>% multi_resp_plot(s = "What are the reasons for continuing to own this property today?")
```
<center> 
<span style="color:grey">n = `r srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("RO")), isnt.na) %>% 
  select(starts_with("RO")) %>% filter(RO_1 | RO_2 | RO_3| RO_4| RO_5| RO_6| RO_7|RO_8|RO_9|RO_10|RO_11|RO_12) %>% nrow()`</span>
</center>
...
srpos %>% mutate(Voucher = factor(dplyr::recode(ifelse(is.na(RNTSUB_1), RNTSUB_5,RNTSUB_1), 
                                                "The Federal Section 8 Housing Choice certificate or voucher program" = "Section 8 Housing Choice vouchers"))) %>% 
...
srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("BUY")), isnt.na) %>%
  select(starts_with("BUY")) %>% filter(BUY_1 | BUY_2 | BUY_3| BUY_4| BUY_5) %>% 
  transmute(Electricity = BUY_1,
            Gas = BUY_2,
            `Fuel oil` = BUY_3,
            Water = BUY_4,
            `Trash Collection` = BUY_5) %>% multi_resp_plot(s= "What utilities are included in rent?", t="Other than Trash Collection and Water, Utilities are Rarely Included in Rent", title_reduce = 3)
```
<center> 
<span style="color:grey">n = `r srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("BUY")), isnt.na) %>%
  select(starts_with("BUY")) %>% filter(BUY_1 | BUY_2 | BUY_3| BUY_4| BUY_5) %>% nrow()`</span>
</center>
...
srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("RF")), isnt.na) %>% 
  select(starts_with("RF")) %>% filter(RF_1 | RF_2 | RF_3| RF_4| RF_5|RF_6|RF_8|RF_10|RF_12) %>% 
  transmute(`Last years rent plus inflation adjustment` = RF_1,
            ...
            Other = RF_12) %>% multi_resp_plot(s = "What are the most important factors considered when setting rents at the surveyed property?", t= "Sampled Owners Considered a Wide Range of Factors When Setting Rents", sub_reduce = 20, title_reduce = 20)
```
<center> 
<span style="color:grey">n = `r srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("RF")), isnt.na) %>% 
  select(starts_with("RF")) %>% filter(RF_1 | RF_2 | RF_3| RF_4| RF_5|RF_6|RF_8|RF_10|RF_12) %>% nrow()`</span>
</center>
...
srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("Q156")), isnt.na) %>% 
  select(starts_with("Q156")) %>% filter(Q156_1 | Q156_3 | Q156_4| Q156_6) %>% 
  transmute(`Property management software` = Q156_1,
            `Microsoft Excel or similar spreadsheet software` = Q156_3,
            `Tools on property management / landlord websites` = Q156_4,
            `No technological tools are used to manage this property` = Q156_6) %>% multi_resp_plot(s= "Which if any of the following types of technological tools are used to help manage the property?", sub_reduce = 10)
```
<center> 
<span style="color:grey">n = `r srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("Q156")), isnt.na) %>% 
  select(starts_with("Q156")) %>% filter(Q156_1 | Q156_3 | Q156_4| Q156_6) %>% nrow()`</span>
</center>

```{r}
srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("ManagerRole")), isnt.na) %>% 
  select(starts_with("ManagerRole")) %>% filter(ManagerRole_1 | ManagerRole_2 | ManagerRole_3| ManagerRole_4|ManagerRole_5|ManagerRole_6|ManagerRole_7|ManagerRole_8) %>% 
  transmute(`Collect rent` = ManagerRole_1,
            ...
            `Initiate legal actions other than evictions` = ManagerRole_8) %>% multi_resp_plot(s = "What activities does the manager or management company perform?", t= "Property Managers Most Often Performed Rent Collection, Tenant Selection, Routine Maintenance and Eviction Activities", title_reduce = 10)
```

<center> 
<span style="color:grey">n = `r srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("ManagerRole")), isnt.na) %>% 
  select(starts_with("ManagerRole")) %>% filter(ManagerRole_1 | ManagerRole_2 | ManagerRole_3| ManagerRole_4|ManagerRole_5|ManagerRole_6|ManagerRole_7|ManagerRole_8) %>% nrow()`</span>
</center>
...
srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("UPGD")), isnt.na) %>% 
  select(starts_with("UPGD")) %>% filter(UPGD_1 | UPGD_2 | UPGD_3| UPGD_4|UPGD_5|UPGD_6|UPGD_8|UPGD_10) %>% 
  transmute(`Upgrading of heating system` = UPGD_1,
            ...
            Other = UPGD_10) %>% multi_resp_plot(s = "In the last 5 years have any of the following capital improvements or upgrades been made or started at the surveyed property?", t = "Bathroom Renovations were Most Common\nCapital Improvement", sub_reduce = 15, title_reduce = 15)
```

<center> 
<span style="color:grey">n = `r srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("UPGD")), isnt.na) %>% 
  select(starts_with("UPGD")) %>% filter(UPGD_1 | UPGD_2 | UPGD_3| UPGD_4|UPGD_5|UPGD_6|UPGD_8|UPGD_10) %>% nrow()`</span>
</center>

```{r}
srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("WorkReason")), isnt.na) %>% 
  select(starts_with("WorkReason")) %>% filter(WorkReason_1 | WorkReason_2 | WorkReason_3| WorkReason_4|WorkReason_6) %>% 
  transmute(`To be able to charge higher rent`        = WorkReason_1,
            ...
            Other                                     = WorkReason_6) %>% multi_resp_plot(s = "For which of the following reasons has work been done on this rental property in the past five years?", t= "The Most Commonly Cited Reason for Conducting Physical Work on the Property was to Bring the Property up to Owner's Own Standards", sub_reduce = 15, title_reduce = 17)
```

<center> 
<span style="color:grey">n = `r srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("WorkReason")), isnt.na) %>% 
  select(starts_with("WorkReason")) %>% filter(WorkReason_1 | WorkReason_2 | WorkReason_3| WorkReason_4|WorkReason_6) %>% nrow()`</span>
</center>

```{r}
srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("Q150")), isnt.na) %>% 
  select(starts_with("Q150")) %>% filter(Q150_1 | Q150_2 | Q150_3| Q150_4|Q150_5|Q150_6|Q150_7)  %>% 
  transmute(`Cash from personal savings`        = Q150_1,
            ...
            `Online personal finance (SoFi, Lending Club, etc.)`  = Q150_7) %>% multi_resp_plot(s = "What sources of funding did the owner use for work on rental properties?")
```

<center> 
<span style="color:grey">n = `r srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("Q150")), isnt.na) %>% 
  select(starts_with("Q150")) %>% filter(Q150_1 | Q150_2 | Q150_3| Q150_4|Q150_5|Q150_6|Q150_7) %>% nrow()`</span>
</center>
...
srpos %>% mutate(AD_1 = ifelse(is.na(ADNOW_1),ADLATER_1,ADNOW_1),
  ...
  select(starts_with("AD")) %>% filter(AD_1 | AD_3 | AD_4| AD_5|AD_6|AD_7|AD_8|AD_9) %>% 
  transmute(`Newspaper ads`                                    = AD_1,
            ...
            Other                                              = AD_8,
            `A rental broker`                                  = AD_9) %>% multi_resp_plot(s = "Which of the following types of advertising are used to market the property?")
```

<center> 
<span style="color:grey">n = `r srpos %>% mutate(AD_1 = ifelse(is.na(ADNOW_1),ADLATER_1,ADNOW_1),
                 AD_3 = ifelse(is.na(ADNOW_3),ADLATER_3,ADNOW_3),
                 AD_4 = ifelse(is.na(ADNOW_4),ADLATER_4,ADNOW_4),
                 AD_5 = ifelse(is.na(ADNOW_5),ADLATER_5,ADNOW_5),
                 AD_6 = ifelse(is.na(ADNOW_6),ADLATER_6,ADNOW_6),
                 ...
                 AD_9 = ifelse(is.na(ADNOW_9),ADLATER_9,ADNOW_9)) %>%
  select(-starts_with("ADNOW"),-starts_with("ADLATER")) %>% mutate_at(vars(starts_with("AD")), isnt.na) %>% 
  select(starts_with("AD")) %>% filter(AD_1 | AD_3 | AD_4| AD_5|AD_6|AD_7|AD_8|AD_9)  %>% nrow()`</span>
</center>

```{r}
srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("SC")), isnt.na) %>% 
  select(starts_with("SC")) %>% filter(SC_1 | SC_2| SC_3 | SC_4| SC_5|SC_6|SC_7|SC_8|SC_9) %>% 
  transmute(`Personal interviews`        = SC_1,
            ...
            `Online screening service (e.g. RentPrep)`  = SC_9) %>% multi_resp_plot(s= "Have any of the following methods been used to screen potential tenants for this property?", t = "Owners Use Many Different Criteria in Tenant Selection", sub_reduce = 5)
```

<center> 
<span style="color:grey">n = `r srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("SC")), isnt.na) %>% 
  select(starts_with("SC")) %>% filter(SC_1 | SC_2| SC_3 | SC_4| SC_5|SC_6|SC_7|SC_8|SC_9)%>% nrow()`</span>
</center>

```{r}
srpos %>% mutate(SCIMPORT = dplyr::recode(SCIMPORT, "References from previous rental agent/owner/property owners association" = "References from previous rental agent/\nowner/property owners association",
                                          "Some other means  - Please specify" = "Some other means")) %>% binned_bar(SCIMPORT,"Which method is the most important in your selection process?")
srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("Q135")), isnt.na) %>% 
  select(starts_with("Q135")) %>% filter(Q135_1 | Q135_2| Q135_3 | Q135_4| Q135_5|Q135_6) %>% 
  transmute(`Evidence that the tenant has and will continue to have income to pay rent`        = Q135_1,
            ...
            `Don't know`  = Q135_5,
            Other  = Q135_6) %>% multi_resp_plot(s = "What criteria are used when selecting tenants for this property?", t = "Ability to Pay is the Most Common Criteria for Tenant Selection", title_reduce = 10)
```

<center> 
<span style="color:grey">n = `r srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("Q135")), isnt.na) %>% 
  select(starts_with("Q135")) %>% filter(Q135_1 | Q135_2| Q135_3 | Q135_4| Q135_5|Q135_6) %>% nrow()`</span>
</center>
...
srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("KPwhy")), isnt.na) %>% 
  select(starts_with("KPwhy")) %>% filter(KPwhy_2 | KPwhy_3| KPwhy_4) %>% 
  transmute(`To retain desirable tenants` = KPwhy_2,
            `To minimize turnover costs`  = KPwhy_3,
            `To lower maintenance costs`  = KPwhy_4) %>% multi_resp_plot(s= "Why is the management trying to minimize tenant turnover at this property?", t = "Most Owners Trying to Reduce Turnover do so to Keep Good Tenants and Reduce Costs", title_reduce = 10)
```

<center> 
<span style="color:grey">n = `r srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("KPwhy")), isnt.na) %>% 
  select(starts_with("KPwhy")) %>% filter(KPwhy_2 | KPwhy_3| KPwhy_4)%>% nrow()`</span>
</center>

```{r}
srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("KPhow")), isnt.na) %>% 
  select(starts_with("KPhow")) %>% filter(KPhow_1 | KPhow_2| KPhow_3| KPhow_4 | KPhow_5) %>% 
  transmute(`Rent concessions or reductions`            = KPhow_1,
            ...
            `Improving services to the tenants`         = KPhow_5) %>% multi_resp_plot(s = "What techniques are used to minimize tenant turnover at this property?", t = "Owners Use a Variety of Methods to Reduce Turnover")
```

<center> 
<span style="color:grey">n = `r srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("KPhow")), isnt.na) %>% 
  select(starts_with("KPhow")) %>% filter(KPhow_1 | KPhow_2| KPhow_3| KPhow_4 | KPhow_5)%>% nrow()`</span>
</center>
...
srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("S8")), isnt.na) %>% 
  select(starts_with("S8")) %>% filter(S8_1 | S8_2| S8_3| S8_4 | S8_5| S8_6|S8_7)  %>% 
  transmute(`Concerned about ability to collect on the vouchers or certificates`            = S8_1,
            ...
            Other = S8_7) %>% multi_resp_plot(s = "Which of the following reasons describe why you do not want to accept new voucher tenants at this property?", sub_reduce = 20)
```
<center> 
<span style="color:grey">n = `r srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("S8")), isnt.na) %>% 
  select(starts_with("S8")) %>% filter(S8_1 | S8_2| S8_3| S8_4 | S8_5| S8_6|S8_7)%>% nrow()`</span>
</center>
...
srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("DLQ")), isnt.na) %>% 
  select(starts_with("DLQ")) %>% filter(DLQ_1 | DLQ_2 | DLQ_3 | DLQ_4) %>% 
  transmute(`By notifying the tenants that they are late before taking further action`        = DLQ_1,
  ...
  multi_resp_plot(s = "In the past 2 years, in what ways have you dealt with tenants who are delinquent in their rent payments?", sub_reduce = 10)
```
<center> 
<span style="color:grey">n = `r srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("DLQ")), isnt.na) %>% 
  select(starts_with("DLQ")) %>% filter(DLQ_1 | DLQ_2 | DLQ_3 | DLQ_4) %>% nrow()`</span>
</center>
...
srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("Q46")), isnt.na) %>% 
  select(starts_with("Q46")) %>% filter(Q46_1 | Q46_2 | Q46_3 | Q46_4| Q46_5 | Q46_6 | Q46_7 | Q46_8)  %>% 
  transmute(`Vandalism to the INSIDE of unit(s)`                      = Q46_1,
            ...
            Other                                                     = Q46_8) %>%
  multi_resp_plot(s = "In the past 2 years, did any of the following happen in any of the units or on the grounds of the property?", t = "Disruptive Behavior in Surveyed Properties Was Rare, but Took Many forms When it Happened", sub_reduce = 10, title_reduce = 10)
```
<center> 
<span style="color:grey">n = `r srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("Q46")), isnt.na) %>% 
  select(starts_with("Q46")) %>% filter(Q46_1 | Q46_2 | Q46_3 | Q46_4| Q46_5 | Q46_6 | Q46_7 | Q46_8) %>% nrow()`</span>
</center>

```{r}
srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("DRP")), isnt.na) %>% 
  select(starts_with("DRP")) %>% filter(DRP_1 | DRP_2 | DRP_5 | DRP_6| DRP_7 | DRP_8) %>% 
  transmute(`By talking to the disruptive individuals in person`                          = DRP_1,
            ...
            Other                                                                         = DRP_8) %>%
  multi_resp_plot(s = "In the past 2 years, HOW have you dealt with undesirable or disruptive behavior at this property?", sub_reduce = 10)
```
<center> 
<span style="color:grey">n = `r srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("DRP")), isnt.na) %>% 
  select(starts_with("DRP")) %>% filter(DRP_1 | DRP_2 | DRP_5 | DRP_6| DRP_7 | DRP_8) %>% nrow()`</span>
</center>
...
srpos %>% filter(InPopulation == 1) %>% 
  select(starts_with("Q55")) %>% 
  transmute(`Lead-based paint requirements`                    = Q55_1,
  ...
  gather("kind", "val") %>% drop_na(val) %>% ggplot(aes(reorder(kind, val), fill=fct_relevel(val,"No", "Yes, a little", "Yes, somewhat", "Yes, a lot"))) + 
  geom_bar(position = "fill") + aes(reorder(stringr::str_wrap(kind, 30),val)) + xlab(NULL) +theme(axis.title.x = element_blank())+
...
srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("FINANCED")), isnt.na) %>% 
  select(starts_with("FINANCED")) %>% filter(FINANCED_1 | FINANCED_2 | FINANCED_3 | FINANCED_4| FINANCED_6 | FINANCED_7 | FINANCED_8 | FINANCED_9 | FINANCED_10) %>%
  transmute(`Placed one new mortgage (land contract, etc.)`       = FINANCED_1,
  ...
  multi_resp_plot(s = "How was the acquisition of the property financed?")
```
<center> 
<span style="color:grey">n = `r srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("FINANCED")), isnt.na) %>% 
  select(starts_with("FINANCED")) %>% filter(FINANCED_1 | FINANCED_2 | FINANCED_3 | FINANCED_4| FINANCED_6 | FINANCED_7 | FINANCED_8 | FINANCED_9 | FINANCED_10) %>% nrow()`</span>
</center>

```{r}
srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("DWNPAY")), isnt.na) %>% 
  select(starts_with("DWNPAY")) %>% filter(DWNPAY_1 | DWNPAY_2 | DWNPAY_3 | DWNPAY_4| DWNPAY_5 | DWNPAY_6 | DWNPAY_7 | DWNPAY_8 | DWNPAY_10 | DWNPAY_11) %>% 
  transmute(`Sale of land or other real estate`       = DWNPAY_1,
  ...
  multi_resp_plot(s = "What was the major source(s) of equity or down payment for this property?", 
                  sub_reduce = 20)
```
<center> 
<span style="color:grey">n = `r srpos %>% filter(InPopulation == 1) %>% mutate_at(vars(starts_with("DWNPAY")), isnt.na) %>% 
  select(starts_with("DWNPAY")) %>% filter(DWNPAY_1 | DWNPAY_2 | DWNPAY_3 | DWNPAY_4| DWNPAY_5 | DWNPAY_6 | DWNPAY_7 | DWNPAY_8 | DWNPAY_10 | DWNPAY_11) %>% nrow()`</span>
</center>

```{r}
srpos %>% mutate(Mortgage = ifelse(is.na(MS_1),MS_5,MS_1)) %>% binned_bar(Mortgage, "Is there CURRENTLY a mortgage (including deed of trust, trust deed, contract to purchase, or land contract), home equity line of credit, or similar debt on this property?")
